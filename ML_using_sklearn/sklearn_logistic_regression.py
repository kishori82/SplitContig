...
def main(argv, errorlogger = None, runstatslogger = None):
	...
	for x in C_values:
		LogisticRegression1 = LogisticRegression(C = x).fit(X_train, y_train)
		print "Logistic Regression training set score with C %f: %f" % (x, LogisticRegression1.score(X_train, y_train))
		test_set_Accuracy = (x,LogisticRegression1.score(X_test,y_test))
		list_test_Accuracy.append(test_set_Accuracy)

	for i,j in list_test_Accuracy:
		print "Logistic Regression test set score with C ={} is: {}".format(i,j)
	print '='*40
